...
def gram_matrix(features, normalize=True):
    ...
    _, H, W, C = f_shape[0], f_shape[1], f_shape[2], f_shape[3]
    f_reshaped = tf.reshape(features, [H*W, C])
    f_reshaped_t = tf.transpose(f_reshaped)
    gram = tf.matmul(f_reshaped_t, f_reshaped)
